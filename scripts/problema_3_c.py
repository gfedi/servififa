...
def main():
    ...
    for t in events['teamId'].unique():
        ...
        for x in range(5):
            for y in range(5):
                ...
                frequenza_passaggi_accurati[x, y] = \
                    accurate_passes_in_the_cell / float(tot_accurate_passes)

                frequenza_tiri[x, y] = shots_in_the_cell / float(tot_shots)

                frequenza_falli_subiti[x, y] = \
                    foul_in_the_cell / float(tot_foul)

        team[t] = {'frequenza_eventi': frequenza_eventi,
    ...
    results = []
    for tipo_griglia in ['frequenza_eventi', 'frequenza_passaggi_accurati',
        ...
        for x in range(5):
            for y in range(5):
                ...
                temp['squadra_dominante'] = events['teamId'].unique()[0] if \
                    team[events['teamId'].unique()[0]][tipo_griglia][x, y] > \
                        team[events['teamId'].unique()[0]][tipo_griglia][x, y] \
